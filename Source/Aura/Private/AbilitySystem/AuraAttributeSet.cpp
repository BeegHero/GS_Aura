...
//Use this method for only clamping, not Gameplay effects.
// Doesn't permanently change the modifier, just the value returned from querying the modifier. 
...
void UAuraAttributeSet::SetEffectProperties(const struct FGameplayEffectModCallbackData& Data, FEffectProperties& Props) const
{
	...
	// Target of the effect, but the owner of this Attribute set
	...
	if (IsValid(Props.SourceASC) && Props.SourceASC->AbilityActorInfo.IsValid() && Props.SourceASC->AbilityActorInfo->AvatarActor.IsValid())
	{
		...
		//If SourceController is null, cast from Pawn
...
void UAuraAttributeSet::PostGameplayEffectExecute(const struct FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetIncomingXPAttribute())
	{
		...
		//Aura has a listen ability that applies a GE with incomingXPAttribute, thats why we're checking the source Char
		if (Props.SourceCharacter->Implements<UPlayerInterface>() && Props.SourceCharacter->Implements<UCombatInterface>())
		{
			...
			if (NumOfLevelUps > 0)
			{
				//TODO Get Attributes points reward and Spell Points Reward
				//TODO there might be a bug here where we level up multiple times, but only grant the attr/spell points for the current level
				...
				// Add to attribute points and spell points
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, float Damage, bool bBlockedHit, bool bCriticalHit) const
{
	if (Props.SourceCharacter != Props.TargetCharacter)
	{
		...
		else if (AAuraPlayerController* TargetPC = Cast<AAuraPlayerController>(Props.TargetCharacter->GetController()))
		{
